...
#               2023 Douglas Rennehan (douglas.rennehan@gmail.com
#
...
# Computes the analytical solution of the BrioWu shock and plots the SPH answer


# Generates the analytical  solution for the BrioWu shock test case
...
# Parameters
gas_gamma = 2.0  # Polytropic index
...
sim = h5py.File("brioWuShock_%04d.hdf5" % snap, "r")
...
B_bin, _, _ = stats.binned_statistic(x, By, statistic="mean", bins=x_bin_edge)
...
#B2_bin, _, _ = stats.binned_statistic(x, By ** 2, statistic="mean", bins=x_bin_edge)
rho_sigma_bin = np.sqrt(rho2_bin - rho_bin ** 2)
v_sigma_bin = np.sqrt(v2_bin - v_bin ** 2)
P_sigma_bin = np.sqrt(P2_bin - P_bin ** 2)
S_sigma_bin = np.sqrt(S2_bin - S_bin ** 2)
u_sigma_bin = np.sqrt(u2_bin - u_bin ** 2)
#B_sigma_bin = np.sqrt(B2_bin - B_bin ** 2)
...
# y-direction magnetic field profile -------------------------
subplot(234)
plot(x, By, ".", color="r", ms=0.2)
#plot(x_s, By_s, "--", color="k", alpha=0.8, lw=1.2)
#errorbar(x_bin, B_bin, yerr=B_sigma_bin, fmt=".", ms=8.0, color="b", lw=1.2)
xlabel("${\\rm{Position}}~x$", labelpad=0)
ylabel("${\\rm{Magnetic~Field}}~By$", labelpad=0)
xlim(-0.5, 0.5)
ylim(-1.2, 1.2)
...
text(
    -0.49,
    0.9,
    "BrioWu shock with  $\\gamma=%.3f$ in 2D at $t=%.2f$" % (gas_gamma, time),
...
tight_layout()
savefig("BrioWuShock.png", dpi=200)
