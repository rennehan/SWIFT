 ...
 *
 ******************************************************************************/
#ifndef SWIFT_DENSITY_REACTIVE_HYDRO_IO_H
#define SWIFT_DENSITY_REACTIVE_HYDRO_IO_H

/**
 * @file DENSITY_REACTIVE/hydro_io.h
 * @brief Density-Energy conservative implementation of SPH,
 *        with added DENSITY_REACTIVE physics (Rennehan 2025) (i/o routines)
 */
...
INLINE static void convert_biased_P(const struct engine* e, const struct part* p,
                                    const struct xpart* xp, float* ret) {

  ret[0] = gas_pressure_from_internal_energy(p->rho_biased, p->u);
}
...
INLINE static void hydro_write_particles(const struct part* parts,
  ...
  *num_fields = 23;
  ...
  list[16] = io_make_output_field(
      "BiasedDensities", FLOAT, 1, UNIT_CONV_DENSITY, -3.f, parts, rho_biased,
      "Co-moving biased mass densities of the particles");
  
  list[17] = io_make_output_field(
      "MeanDensities", FLOAT, 1, UNIT_CONV_DENSITY, -3.f, parts, 
      contrast.rho_mean,
      "Co-moving arithmetic mean mass densities of the particles");

  list[18] = io_make_output_field(
      "DensityChis", FLOAT, 1, UNIT_CONV_NO_UNITS, 0.f, parts, 
      contrast.rho_chi,
      "Density contrast indicator (DR-SPH) of the particles");

  list[19] = io_make_output_field(
      "BalsaraFactors", FLOAT, 1, UNIT_CONV_NO_UNITS, 0.f, parts,
      force.balsara,
      "Balsara f factor");

  list[20] = io_make_output_field_convert_part(
      "BiasedPressures", FLOAT, 1, UNIT_CONV_PRESSURE, -3.f * hydro_gamma, parts,
      xparts, convert_biased_P, "Co-moving biased pressures of the particles");

  list[21] = io_make_output_field(
      "MeanPressures", FLOAT, 1, UNIT_CONV_PRESSURE, -3.f * hydro_gamma, parts, 
      contrast.P_mean,
      "Co-moving arithmetic mean pressures of the particles");

  list[22] = io_make_output_field(
      "PressureChis", FLOAT, 1, UNIT_CONV_NO_UNITS, 0.f, parts, 
      contrast.P_chi,
      "Pressure contrast indicator (DR-SPH) of the particles");
...
#endif /* SWIFT_DENSITY_REACTIVE_HYDRO_IO_H */
