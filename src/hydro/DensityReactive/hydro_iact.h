 ...
 *
 ******************************************************************************/
#ifndef SWIFT_DENSITY_REACTIVE_HYDRO_IACT_H
#define SWIFT_DENSITY_REACTIVE_HYDRO_IACT_H

/**
 * @file DENSITY_REACTIVE/hydro_iact.h
 * @brief Density-Energy conservative implementation of SPH,
 *        with added DENSITY_REACTIVE physics (Borrow 2020) (interaction routines)
 */
...
__attribute__((always_inline)) INLINE static void runner_iact_gradient(
  ...
  pi->diffusion.laplace_u += pj->mass * delta_u_factor * wi_dx * rho_inv_j;
  pj->diffusion.laplace_u -= pi->mass * delta_u_factor * wj_dx * rho_inv_i;
  ...
  /* DR-SPH density kernel properties */
  pi->contrast.rho_min = min(pi->rho, pj->rho);
  pj->contrast.rho_min = min(pj->rho, pi->rho);
  pi->contrast.rho_max = max(pi->rho, pj->rho);
  pj->contrast.rho_max = max(pj->rho, pi->rho);
  pi->contrast.rho_sum += pj->rho;
  pj->contrast.rho_sum += pi->rho;

  const float rho_chi_ji = 
      fabs(log10f(pj->rho * rho_inv_i)) - const_gradient_log10_ratio;
  const float rho_chi_ij = 
      fabs(log10f(pi->rho * rho_inv_j)) - const_gradient_log10_ratio;
  pi->contrast.rho_chi += (rho_chi_ji > 0.f) ? 1.f : 0.f;
  pj->contrast.rho_chi += (rho_chi_ij > 0.f) ? 1.f : 0.f;

  /* DR-SPH pressure kernel properties */
  const float P_i = pi->pressure;
  const float P_j = pj->pressure;
  const float P_inv_i = 1.f / P_i;
  const float P_inv_j = 1.f / P_j;

  pi->contrast.P_min = min(P_i, P_j);
  pj->contrast.P_min = min(P_j, P_i);
  pi->contrast.P_max = max(P_i, P_j);
  pj->contrast.P_max = max(P_j, P_i);
  pi->contrast.P_sum += P_j;
  pj->contrast.P_sum += P_i;

  const float P_chi_ji = 
      fabs(log10f(P_j * P_inv_i)) - const_gradient_log10_ratio;
  const float P_chi_ij = 
      fabs(log10f(P_i * P_inv_j)) - const_gradient_log10_ratio;
  pi->contrast.P_chi += (P_chi_ji > 0.f) ? 1.f : 0.f;
  pj->contrast.P_chi += (P_chi_ij > 0.f) ? 1.f : 0.f;
...
__attribute__((always_inline)) INLINE static void runner_iact_force(
  ...
  /* Rennehan: Use biased rhoi and rhoj in momentum+energy equations. The key 
   * point of the DR-SPH method is to replace the densities with a biased 
   * density if there is an extreme gradient in the kernel. p->contrast.chi 
   * contains the fraction of particles that are more than an order-of-magnitude 
   * away from p's density. */
  const float rhoi_biased = pi->rho_biased;
  const float rhoj_biased = pj->rho_biased;

  const float pressurei_biased = pi->pressure_biased;
  const float pressurej_biased = pj->pressure_biased;
  ...
  /* Rennehan: The key point of the DR-SPH method is to replace the 
   * densities with a biased density if there is an extreme gradient in the
   * kernel. p->contrast.chi contains the fraction of particles that are
   * more than an order-of-magnitude away from p's density. */

  /* Compute gradient terms */
  const float P_over_rho2_i = 
      pressurei_biased / (rhoi_biased * rhoi_biased) * f_ij;
  const float P_over_rho2_j = 
      pressurej_biased / (rhoj_biased * rhoj_biased) * f_ji;
...
__attribute__((always_inline)) INLINE static void runner_iact_nonsym_force(
  ...
  /* Rennehan: Use biased rhoi and rhoj in momentum+energy equations */
  const float rhoi_biased = pi->rho_biased;
  const float rhoj_biased = pj->rho_biased;
...
#endif /* SWIFT_DENSITY_REACTIVE_HYDRO_IACT_H */
