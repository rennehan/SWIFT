 ...
 *
 ******************************************************************************/
#ifndef SWIFT_DENSITY_REACTIVE_HYDRO_H
#define SWIFT_DENSITY_REACTIVE_HYDRO_H

/**
 * @file DENSITY_REACTIVE/hydro.h
 * @brief Density-Energy conservative implementation of SPH,
 *        with added DENSITY_REACTIVE physics (Borrow 2020)  (Non-neighbour loop
 *        equations)
 */
...
__attribute__((always_inline)) INLINE static void hydro_end_density(
  ...
  p->pressure += p->mass * p->u * kernel_root;
  ...
  p->rho *= h_inv_dim;
  p->pressure *= (hydro_gamma_minus_one * h_inv_dim);
...
__attribute__((always_inline)) INLINE static void hydro_reset_gradient(
  ...
  p->contrast.rho_sum = 0.f;
  p->contrast.rho_max = p->rho;
  p->contrast.rho_min = p->rho;
  p->contrast.rho_chi = 0.f;
  p->contrast.P_sum = 0.f;
  p->contrast.P_max = p->force.pressure;
  p->contrast.P_min = p->force.pressure;
  p->contrast.P_chi = 0.f;
  p->contrast.num_ngb = 0;

}

/**
 * @brief Computes the exponentially damped bias term for contrasting x.
 *
 *
 * @param x The quantity 
 * @param x_max The maximum quantity in the kernel
 * @param x_min The minimum quantity in the kernel
 * @param x_mean The mean quantitiy in the kernel
 */
__attribute__((always_inline)) INLINE static float hydro_compute_bias_weight(
    const float x, const float x_max, const float x_min, const float x_mean) {

  float delta_x_num = 0.f;
  float delta_x_den = 1.f;

  if (x > x_mean) {
    delta_x_num = x - x_mean;
    delta_x_den = x_max - x_mean;
  } 
  else {
    delta_x_num = x_mean - x;
    delta_x_den = x_mean - x_min;
  }

  const float x_slope = delta_x_num / delta_x_den;

  /* Exponential suppression of bias near extrema */
  const float x_weight = 
      1.f - expf(-const_gradient_suppression_sharpness * x_slope);
...
/**
 * @brief Computes the biased quantity x.
 *
 *
 * @param x The quantity 
 * @param x_max The maximum quantity in the kernel
 ...
 * @param x_mean The mean quantitiy in the kernel
 */
__attribute__((always_inline)) INLINE static float hydro_compute_biased_quantity(
    const float x, const float x_max, const float x_min, const float x_mean,
    const float f_weight) {

  const float delta = fmaxf(x / x_mean, 1e-5f);

  const float weight = 
      hydro_compute_bias_weight(x, x_max, x_min, x_mean);

  const float sign = (x > x_mean) ? 1.f : -1.f;

  const float alpha = 
      const_gradient_alpha_max * f_weight * weight * sign;

  const float x_biased = x_mean * powf(delta, 1.f + alpha);
...
__attribute__((always_inline)) INLINE static void hydro_end_gradient(
  ...
  p->contrast.rho_sum += p->rho;
  p->contrast.P_sum += p->pressure;
  const float N = (float)p->contrast.num_ngb + 1.f;

  /* Finalize means (no kernel weighting) */
  p->contrast.rho_mean = p->contrast.rho_sum / N;
  p->contrast.P_mean = p->contrast.P_sum / N;

  /* Rho gradient indicator */
  p->contrast.rho_chi /= N;
  p->contrast.P_chi /= N;

  /* Always use unbiased rho when possible */
  p->rho_biased = p->rho;
  p->pressure_biased = p->force.pressure;

  const char bias_rho_flag = 
      (p->contrast.rho_chi > const_rho_gradient_minimum_chi);
  const char bias_P_flag = (p->contrast.P_chi > const_P_gradient_minimum_chi);

  if (bias_rho_flag || bias_P_flag) {
    /* Normalized deviation from the Balsara shear/compression threshold */
    const float delta_f = 
        p->force.balsara - const_balsara_compression_threshold;
    const float delta_f_range = (delta_f >= 0.f)
                                  ? (1.f - const_balsara_compression_threshold)
                                  : const_balsara_compression_threshold;
    const float f_weight = delta_f / delta_f_range;

    /* Apply contrast-biased density */
    if (bias_rho_flag) {
      p->rho_biased = hydro_compute_biased_quantity(p->rho, 
                                                    p->contrast.rho_max,
                                                    p->contrast.rho_min,
                                                    p->contrast.rho_mean,
                                                    f_weight);
    ...
    /* Apply contrast-based pressure */
    if (bias_P_flag) {
      p->pressure_biased = hydro_compute_biased_quantity(p->pressure, 
                                                         p->contrast.P_max,
                                                         p->contrast.P_min,
                                                         p->contrast.P_mean,
                                                         f_weight);
      ...
      p->pressure_biased = hydro_gamma_minus_one * p->rho_biased * p->u;
    }
  }
...
__attribute__((always_inline)) INLINE static void hydro_part_has_no_neighbours(
  ...
  p->rho_biased = p->rho;
  p->pressure = p->mass * p->u * kernel_root * h_inv_dim;
  p->pressure_biased = p->pressure;
...
#endif /* SWIFT_DENSITY_REACTIVE_HYDRO_H */
